...
public class UserService {
	...
	public ATMResponse createAccount(UserDto userDTO) {
		...
		if (userRepository.existsByContact(userDTO.getContact())) {
			return ATMResponse.builder().responseCode(AccountUtils.ACCOUNT_EXIST_CODE)
					.responseMessage(AccountUtils.ACCOUNT_EXIST_MESSAGE).accountDtos(null).build();
		}

		// Create new User
		User newUser = User.builder().name(userDTO.getName()).address(userDTO.getAddress())
				.contact(userDTO.getContact()).pin(userDTO.getPin()).build();
		...
		List<AccountDto> accountDtos = savedUser.getAccounts().stream()
				.map(account -> AccountDto.builder().accountNumber(account.getAccountNumber()).balance(0.0).build())
				.collect(Collectors.toList());

		// Prepare Response
		return ATMResponse.builder().responseCode(AccountUtils.ACCOUNT_CREATION_SUCCESS)
				.responseMessage(AccountUtils.ACCOUNT_CREATION_MESSAGE).accountDtos(accountDtos) // You can set account
																									// details here if
																									// needed
				.build();
	}
